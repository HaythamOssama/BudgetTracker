...
class BackdropLayout @JvmOverloads constructor(context: Context, attribute : AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    private var disableViewDrawableBuilder: DrawableBuilder? = DrawableBuilder().rectangle().apply {
        cornerRadii(
            frontHeaderRadius,
            frontHeaderRadius,
            0,
    ...
    private fun update(withAnimation : Boolean) {
        ...
        when(state) {
            ...
            State.CLOSE -> {
                ...
                getToolbar()?.navigationIcon?.setTint(resources.getColor(R.color.white))
    ...
    private fun getToolbar() : Toolbar? {
        ...
        rootView.findViewById<Toolbar>(toolbarId).setOnClickListener {
            Logger.logDebug("Toolbar clicked!")
        }
        return toolbar
    }

    fun setOverlayDrawable(drawableBuilder: DrawableBuilder?) {
        disableViewDrawableBuilder = drawableBuilder
    ...
    private fun getDisablingView() : View {
        return if( disablingView != null ) disablingView!!
            ...
            if (disableViewDrawableBuilder != null) {
                val drawable = disableViewDrawableBuilder!!.build()
